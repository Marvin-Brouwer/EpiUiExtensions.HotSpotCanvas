...
export default canvasEditorComponent(({ updateValue, element, getConfig, propertyName, isReadonly }) => {
	...
	function mountComponents() {
		...
		const backdrop = element('backdrop');
		const componentWrapper = element('hotspot-hero');
		...
		hotSpotEditor.connect(
			element('hotspot-controls'), element('hotspot-display'), element('insert-controls'), currentValue.hotSpots, settings,
			handleHotSpotChange
		);
	}
	...
	function unmountComponents() {
		...
		element("hotspot-controls").innerHTML = '';
	}
